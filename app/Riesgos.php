...
class Riesgos extends Model
{
  ...
  public function gradoMorales($id)
  {
    ...
    $gedad = Moral::where('id', $id)->first()->personasMorales[0]->date_birth;
    ...
    $pactividad = DB::SELECT("select factor,
                      ...
                      *(ponderacion/100)),2) as resultado
                      from ponderacion
                      LEFT JOIN perfil_transacional_moral on perfil_transacional_moral.id_moral=$id
                      LEFT JOIN actividad_giro on actividad_giro.id=perfil_transacional_moral.actividad_giro
                      LEFT JOIN profesion on profesion.id=perfil_transacional_moral.profesion
    ...
    $porigen = DB::SELECT("select factor,
                                ...
                                *(ponderacion/100)),2) as resultado
                                from ponderacion
                                LEFT JOIN perfil_transacional_moral on perfil_transacional_moral.id_moral=$id
                                LEFT JOIN origen_recursos on origen_recursos.id=perfil_transacional_moral.origen_recursos
                                LEFT JOIN divisa on divisa.id=perfil_transacional_moral.divisas
                                LEFT JOIN instrumento_monetario on instrumento_monetario.id=perfil_transacional_moral.instrumento_monetario
                                where tipo='origen'");
    $pantecedentes = DB::SELECT("select factor,
                            ...
                            *(ponderacion/100)),2) as resultado
                            from ponderacion
                            LEFT JOIN morales on morales.id=$id
                            LEFT JOIN perfil_transacional_moral on perfil_transacional_moral.id_moral=$id
                            LEFT JOIN pld on pld.id=perfil_transacional_moral.pId
                            ...
                            LEFT JOIN entidad_federativa_residencia on entidad_federativa_residencia.id=perfil_transacional_moral.efr
    ...
    $pdestino = DB::SELECT("select factor, descripcion, ponderacion, puntaje, format((puntaje*(ponderacion/100)),2) as resultado
                            from ponderacion
                            LEFT JOIN perfil_transacional_moral on perfil_transacional_moral.id_moral=$id
                            LEFT JOIN destino_recursos on destino_recursos.id=perfil_transacional_moral.destino_recursos
                            where tipo='destino'");
    $antecedentesres = 0;
    $antecedentespon = 50;
    ...
    return json_encode($totalrespon);
  }
